...
def day_of(ts_us: int) -> str:
    ...
    return dt.datetime.fromtimestamp(ts_us // 1_000_000, dt.timezone.utc).strftime("%Y-%m-%d")


def by_unit(records: list[dict[str, str | int]]) -> list[tuple[str, int]]:
...
def by_day(records: list[dict[str, str | int]]) -> list[tuple[str, int]]:
